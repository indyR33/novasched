    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0F172A]/40 backdrop-blur-xs animate-in fade-in">
      <div className="bg-white rounded-2xl shadow-xl border border-[#E2E8F0] max-w-lg w-full overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-[#E2E8F0] flex items-center justify-between bg-[#F8FAFC]">
          <div>
            <h3 className="text-base font-bold text-[#1E293B]">Modifier l'affectation</h3>
        ...
        <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
                  ...
                  >
                    ...
                    {!isAuthorized && (
                      <div className="mt-1 text-[10px] text-rose-600 font-medium flex items-center gap-1">
                        ...
                        <span className="truncate">{reason || 'Non habilité'}</span>
                      </div>
                    )}
                  </button>
                );
          ...
          {selectedShift && (
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs space-y-1.5">
              <div className="flex justify-between items-center text-slate-700">
                <span className="font-medium">Amplitude horaire :</span>
                <span className="font-mono">
                  {selectedShift.startTime} → {selectedShift.endTime}{' '}
                  {selectedShift.isOvernight && '(Nuit - traverse minuit)'}
                </span>
              </div>
              <div className="flex justify-between items-center text-slate-700">
                <span className="font-medium">Heures théoriques vs comptabilisées (R15) :</span>
                <span className="font-mono">
                  {selectedShift.theoreticalDuration}h théoriques |{' '}
                  <strong className="text-indigo-700">{selectedShift.countedHours}h comptabilisées</strong>
                </span>
              </div>
            </div>
          )}
          ...
          {requiresOverride && (
            <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-300 text-xs space-y-2.5">
              <div className="flex items-start gap-2 text-amber-900 font-semibold">
                ...
                <div>
                  <div>Alerte de Violation de Règle HARD (R01 / R02)</div>
                  <p className="font-normal text-amber-800 text-[11px] mt-0.5">
                    {refusalReason || 'Cet employé n’est pas qualifié pour ce shift selon le référentiel.'}
                  </p>
                </div>
              </div>
              ...
              {userRole === 'ADMIN' ? (
                <div className="space-y-2 pt-1 border-t border-amber-200/80">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    ...
                    <span className="font-bold text-amber-950">
                      Appliquer un Override Dérogatoire Exceptionnel (Traçabilité obligatoire)
                    </span>
                  </label>
                  ...
                  {isOverride && (
                    <div>
                      <label className="block text-[11px] font-semibold text-amber-900 mb-1">
                        Motif explicite obligatoire (Règle R36) :
                      </label>
                      <input
                        type="text"
                        placeholder="Ex: Remplacement d’urgence validé par la direction..."
                      ...
                      {(!overrideReason || overrideReason.trim().length < 5) && (
                        <p className="text-[10px] text-rose-600 mt-1">
                          Veuillez saisir un motif justificatif (min 5 caractères).
                        </p>
                      )}
                    </div>
                  )}
                ...
                <p className="text-[11px] text-rose-700 font-medium">
                  Seul un Administrateur peut autoriser une dérogation exceptionnelle pour ce shift.
                </p>
              )}
            </div>
          )}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 mb-1">
              Commentaire / Annotation (Règle R28)
            </label>
            <input
              type="text"
              placeholder="Note opérationnelle ou précision..."
              value={comment}
